...
function endOccupied(array) {
  let count;
  let shift1 = shiftSeats(array);
  count = countOccupied(shift1);
  let shift2 = shiftSeats(shift1);
  count = countOccupied(shift2);
  return count;

}

console.log(endOccupied(inputArray))

function totalSeats(array) {
  let count = 0;
...
function shiftSeats(array) {
  for(let i = 0; i < array.length; i++) {
    ...
    for(let j= 0; j < row.length; j++) {
      ...
      if(array[i-1]) {
        adjacents.push(array[i-1][j]);
        if(row[j+1]) {
          adjacents.push(array[i-1][j+1]);
        }
        if(row[j-1]) {
          adjacents.push(array[i-1][j-1])
        }
