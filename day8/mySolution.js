...
function game(array) {
  ...
  while (currentIdx < array.length) {
    if(set.has(currentIdx)) { //this ends the game
      return accValue;
    } else {
    set.add(currentIdx);
    move = array[currentIdx][0];
    val = array[currentIdx][1];
    if(move === 'acc' && val[0] === '+') {
      accValue += Number(val.slice(1));
      currentIdx++;
    }
    if(move === 'acc' && val[0] === '-') {
      accValue -= Number(val.slice(1));
...
let testArray = [['nop','+0'],['acc','+1'],['jmp','+4'],['acc','+3'],['jmp','-3'],['acc','-99'],['acc','+1'],['jmp','-4'],['acc','+6']];
...
function game2(array) {
  ...
  let val;
  while (currentIdx <= array.length) {
    if(currentIdx === array.length) { //this ends the game / index = 626
      return accValue;
    } else {
    move = array[currentIdx][0];
    val = array[currentIdx][1];
