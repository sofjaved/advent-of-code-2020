...
function validPassports(array) {
  let count = 0;
  for(let i = 0; i < array.length; i++) {
    let passport = array[i];
    if(isValid(passport)) {
      count++
    }
  }
...
console.log(validPassports(passportsArray)); // 233

//part 2

function isValid2(string) {
  let passportObject = objectify(string);
  let keysArr = Object.keys(passportObject);
  if(!keysArr.includes('byr') || !keysArr.includes('iyr') || !keysArr.includes('eyr') || !keysArr.includes('hgt') || !keysArr.includes('hcl') || !keysArr.includes('ecl') || !string.includes('pid')) {
    return false;
  } else if(!byrValid(passportObject['byr']) || !iyrValid(passportObject['iyr']) || !eyrValid(passportObject['eyr']) || !hgtValid(passportObject['hgt']) || !hclValid(passportObject['hcl']) || !eclValid(passportObject['ecl']) || !pidValid(passportObject['pid'])) {
     return false;
   } else {
     return true;
   }
 ...
 function validPassports2(array) {
  let count = 0;
  for(let i = 0; i < array.length; i++) {
    let passport = array[i];
...
console.log(validPassports2(passportsArray)); // 111
...
function byrValid(string) {
  if(string.length === 4 && Number(string) >= 1920 && Number(string) <= 2002) {
    return true;
  } else {
    return false;
  }
...
function iyrValid(string) {
  if(string.length === 4 && Number(string) >= 2010 && Number(string) <= 2020) {
    return true;
  } else {
    return false;
  }
...
function eyrValid(string) {
  if(string.length === 4 && Number(string) >= 2020 && Number(string) <= 2030) {
    return true;
  } else {
    return false;
  }
...
function hgtValid(string) {
  let back = string.slice(string.length - 2);
  if (back !== 'cm' && back !== 'in') {
    return false;
  }
  ...
  for(let i = 0; i < front.length; i++) {
    if(!nums.includes(front[i])) {
      return false;
    }
  }
  if(back === 'cm' && Number(front) >= 150 && Number(front) <= 193) {
    return true;
  } else if (back === 'in' && Number(front) >= 59 && Number(front) <= 76) {
    return true;
  } else {
    return false;
  }
...
function hclValid(string) {
  let reqs = '0123456789abcdef'
  if(string[0] !== '#' || string.length !== 7) {
    return false;
  }
  for(let i = 1; i < string.length; i++) {
    if(!reqs.includes(string[i])) {
      return false;
    }
  }
  return true;
